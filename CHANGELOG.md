...
- **Grafana "Host CPU / Memory" dashboard row** (requires `--collector.system`): a collapsed row with CPU utilization by mode (`idle` excluded and normalized across cores so the stack reads true 0-100% instead of always filling to 100%), normalized busy across cores, load average, memory/swap usage, a full-width per-CPU timeseries with one line per logical core to spot an unbalanced load, and — below the per-CPU panel — **PostgreSQL Process Group CPU** (cores) and **PostgreSQL Process Group Memory** (PSS/RSS bytes, with the live `postgres*` process count on a second axis). The percent panels pin the axis to 0-100%. The **Exporter Self-Monitoring** row remains last.
