...
* `--collector.temp` [temp](src/collectors/temp/README.md) - **Live** temporary-file footprint from `pg_ls_tmpdir()` (PostgreSQL 12+), labeled by `tablespace`: `pg_temp_files_current_bytes`, `pg_temp_files_current_count`, `pg_temp_files_oldest_age_seconds`. `pg_stat_database_temp_bytes` and the `pg_stat_statements` temp counters are *cumulative* and only update when a statement finishes, so a query that is still spilling is invisible to them — these gauges show what is on disk right now and are what catches a runaway sort before it fills the volume. Requires `pg_monitor` (or superuser); degrades to a warn-once no-op without it. Fixed low cardinality; reads only the shared pool. See the [temp-pressure diagnostics](docs/diagnosing-database-pressure.md#9-temporary-file-disk-pressure).
