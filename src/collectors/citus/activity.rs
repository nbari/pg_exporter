    ...
    fn collect<'a>(&'a self, pool: &'a PgPool) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            let query_span = info_span!(
                ...
                db.operation = "SELECT",
                db.statement = "SELECT COALESCE(state, 'unknown') AS state, COUNT(*)::bigint AS count FROM citus_dist_stat_activity WHERE is_worker_query = false GROUP BY state",
                db.sql.table = "citus_dist_stat_activity"
            );

            let rows = match sqlx::query(
                r"SELECT COALESCE(state, 'unknown') AS state, COUNT(*)::bigint AS count
