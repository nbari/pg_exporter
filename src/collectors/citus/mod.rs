...
pub mod stat_tenants;
pub mod tables;

use activity::CitusActivityCollector;
use lock_waits::CitusLockWaitsCollector;
use nodes::CitusNodesCollector;
use schemas::CitusSchemasCollector;
use shards::CitusShardsCollector;
use stat_counters::CitusStatCountersCollector;
use stat_statements::CitusStatStatementsCollector;
use stat_tenants::CitusStatTenantsCollector;
    ...
    pub fn new() -> Self {
        Self {
            subs: vec![
                ...
                Arc::new(CitusLockWaitsCollector::new()),
                Arc::new(CitusSchemasCollector::new()),
                Arc::new(CitusStatStatementsCollector::new()),
                Arc::new(CitusStatTenantsCollector::new()),
            ],
        }
    }
}
    ...
    fn collect<'a>(&'a self, pool: &'a PgPool) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            ...
            while let Some(res) = tasks.next().await {
                if let Err(e) = res {
                    warn!("citus sub-collector error (skipping): {e}");
                }
            }
