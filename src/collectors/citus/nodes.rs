    ...
    fn collect<'a>(&'a self, pool: &'a PgPool) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            let query_span = info_span!(
                ...
                db.operation = "SELECT",
                db.statement = "SELECT nodeid::integer, nodename, nodeport::integer, isactive, noderole::text, shouldhaveshards FROM pg_dist_node",
                db.sql.table = "pg_dist_node"
            );

            let rows = match sqlx::query(
                r"SELECT nodeid::integer, nodename, nodeport::integer, isactive, noderole::text, shouldhaveshards
                  FROM pg_dist_node",
            )
            .fetch_all(pool)
