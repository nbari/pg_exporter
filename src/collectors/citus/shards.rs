    ...
    fn collect<'a>(&'a self, pool: &'a PgPool) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            let query_span = info_span!(
                ...
                db.operation = "SELECT",
                db.statement = "SELECT table_name::text, shardid::bigint, nodename, nodeport::integer, shard_size::bigint FROM citus_shards",
                db.sql.table = "citus_shards"
            );

            let rows = match sqlx::query(
                r"SELECT table_name::text, shardid::bigint, nodename, nodeport::integer, shard_size::bigint
                  FROM citus_shards",
            )
            .fetch_all(pool)
