    ...
    fn collect<'a>(&'a self, pool: &'a PgPool) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            let query_span = info_span!(
                ...
                db.operation = "SELECT",
                db.statement = "SELECT datname, connection_establishment_succeeded::bigint, connection_establishment_failed::bigint, connection_reused::bigint, query_execution_single_shard::bigint, query_execution_multi_shard::bigint FROM citus_stat_counters",
                db.sql.table = "citus_stat_counters"
            );

            let rows = match sqlx::query(
                r"SELECT datname,
                         connection_establishment_succeeded::bigint,
                         connection_establishment_failed::bigint,
                         connection_reused::bigint,
                         query_execution_single_shard::bigint,
                         query_execution_multi_shard::bigint
            ...
            .await
            {
                ...
                Err(e) => {
                    let msg = e.to_string();
                    if msg.contains("citus_stat_counters")
                        || msg.contains("does not exist")
                    {
                        debug!("citus_stat_counters query failed, skipping (requires Citus 12+, schema may vary): {msg}");
                        return Ok(());
                    }
                    return Err(e.into());
                }
