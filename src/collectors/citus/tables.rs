    ...
    fn collect<'a>(&'a self, pool: &'a PgPool) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            let query_span = info_span!(
                ...
                db.operation = "SELECT",
                db.statement = "SELECT table_name::text, citus_table_type::text, shard_count::bigint, citus_table_size(table_name)::bigint AS size_bytes FROM citus_tables",
                db.sql.table = "citus_tables"
            );

            let rows = match sqlx::query(
                r"SELECT table_name::text, citus_table_type::text, shard_count::bigint,
                         citus_table_size(table_name)::bigint AS size_bytes
                  FROM citus_tables",
            )
            .fetch_all(pool)
