...
async fn start_citus_container(image_tag: &str) -> Result<ContainerAsync<GenericImage>> {
    GenericImage::new("citusdata/citus", image_tag)
        ...
        .with_cmd(vec![
            "postgres".to_string(),
            "-c".to_string(),
            "shared_preload_libraries=citus,pg_stat_statements".to_string(),
        ])
        .start()
        .await
        .map_err(Into::into)
...
async fn setup_citus_test_data(pool: &PgPool) -> Result<()> {
    ...
    // Enable pg_stat_statements for citus_stat_statements view
...
fn verify_core_metrics(families: &[MetricFamily]) -> Result<()> {
    // Verify node metrics
...
fn verify_optional_metrics(families: &[MetricFamily]) -> Result<()> {
    ...
    let stat_counter_samples =
        gauge_vec_sample_count(families, "citus_connection_establishment_succeeded");
    ...
    // Verify lock_waits metrics (citus_lock_waits view, gracefully skipped if absent)
    let lock_waits_total = int_gauge_value(families, "citus_lock_waits_total");
    if lock_waits_total.is_some() {
        ensure!(
            lock_waits_total.is_some_and(|v| v >= 0.0),
            "citus_lock_waits_total should be >= 0, got {lock_waits_total:?}"
        );
    }

    // Verify schemas metrics (citus_schemas view, requires Citus 12+, gracefully skipped)
    let schemas_total = int_gauge_value(families, "citus_schemas_total");
    if schemas_total.is_some() {
        ensure!(
            schemas_total.is_some_and(|v| v >= 0.0),
            "citus_schemas_total should be >= 0, got {schemas_total:?}"
        );
    }

    // Verify stat_statements metrics (requires pg_stat_statements, gracefully skipped)
    let stat_statements_total = int_gauge_value(families, "citus_stat_statements_total");
    if stat_statements_total.is_some() {
        ensure!(
            stat_statements_total.is_some_and(|v| v >= 0.0),
            "citus_stat_statements_total should be >= 0, got {stat_statements_total:?}"
        );
    }

    // Verify stat_tenants metrics (citus_stat_tenants view, gracefully skipped)
    let stat_tenants_total = int_gauge_value(families, "citus_stat_tenants_total");
    if stat_tenants_total.is_some() {
        ensure!(
            stat_tenants_total.is_some_and(|v| v >= 0.0),
            "citus_stat_tenants_total should be >= 0, got {stat_tenants_total:?}"
        );
    }
...
fn verify_metric_values_are_finite(families: &[MetricFamily]) -> Result<()> {
    for family in families {
        if !family.name().starts_with("citus_") {
            continue;
        }
        for metric in family.get_metric() {
            let value = metric.get_gauge().value();
                ...
                value.is_finite(),
                "metric {} has non-finite value (NaN or Inf)",
                family.name()
            );
        }
    }
...
fn verify_metric_values_non_negative(families: &[MetricFamily]) -> Result<()> {
    for family in families {
        if !family.name().starts_with("citus_") {
            continue;
        }
        for metric in family.get_metric() {
            let value = metric.get_gauge().value();
                ...
                value >= 0.0,
                "metric {} has negative value {value}",
                family.name()
            );
        }
    }
...
async fn run_citus_collector_test(image_tag: &str) -> Result<()> {
    ...
    verify_core_metrics(&families)?;
    verify_optional_metrics(&families)?;
    verify_metric_values_are_finite(&families)?;
    verify_metric_values_non_negative(&families)?;
...
#[tokio::test]
async fn citus_topology_citus_14() -> Result<()> {
    run_citus_collector_test("14.0.0").await
}

#[tokio::test]
async fn citus_collector_handles_empty_tables() -> Result<()> {
    let test_name = "citus_empty_tables";
    if !ensure_container_runtime_for_test(test_name)? {
        return Ok(());
    }
    ...
    let container = match start_citus_container("12.1.3").await {
        Ok(container) => container,
        Err(error) => {
            if require_runtime {
                return Err(error);
            }
            eprintln!("Skipping citus empty tables test: {error}");
            return Ok(());
        }
    };
    ...
    // Only create extensions — no tables, no data
    sqlx::query("CREATE EXTENSION IF NOT EXISTS citus")
        .execute(&pool)
        .await?;
    sqlx::query("CREATE EXTENSION IF NOT EXISTS pg_stat_statements")
    ...
    // Run the collector against an empty Citus cluster
    let registry = Registry::new();
    let collector = CitusCollector::new();
    collector.register_metrics(&registry)?;
    collector.collect(&pool).await?;
    ...
    // Verify totals are zero (no tables, no shards, no activity)
    let tables_total = int_gauge_value(&families, "citus_tables_total");
    ensure!(
        tables_total.is_some_and(|v| v == 0.0),
        "citus_tables_total should be 0 with no distributed tables, got {tables_total:?}"
    );

    let shards_total = int_gauge_value(&families, "citus_shards_total");
    ensure!(
        shards_total.is_some_and(|v| v == 0.0),
        "citus_shards_total should be 0 with no distributed tables, got {shards_total:?}"
    );

    let activity_total = int_gauge_value(&families, "citus_dist_activity_total");
    ensure!(
        activity_total.is_some(),
        "citus_dist_activity_total should exist even with empty data"
    );

    // All metrics should still be finite and non-negative
    verify_metric_values_are_finite(&families)?;
    verify_metric_values_non_negative(&families)?;
